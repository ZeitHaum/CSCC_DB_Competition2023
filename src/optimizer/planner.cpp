...
bool Planner::get_index_cols(const std::string& tab_name, std::vector<Condition>& curr_conds, std::vector<std::string>& index_col_names) {
    ...
    std::vector<int> vis_mask(curr_conds.size(), 0);
    TabMeta& tab = sm_manager_->db_.get_table(tab_name);

    bool ret = tab.is_leftmost_match(curr_conds, vis_mask ,permutation, index_col_names);
    ...
    //补全permutation
    int begin_ptr = permutation.size();
    for(auto iter = curr_conds.begin(); iter!= curr_conds.end(); iter++){
        //vis_mask: 不在索引中的字段为0, 若有多个取第一个
        if(vis_mask[iter- curr_conds.begin()] == 0){
            permutation[begin_ptr++] = iter - curr_conds.begin();
        }
    }

    std::vector<Condition> target_conds;
    //根据置换生成target
    for(int i = 0; i < (int)curr_conds.size(); i++){
        target_conds.emplace_back(curr_conds[permutation[i]]);
    }

    curr_conds.assign(target_conds.begin(), target_conds.end());
    return true;
}

bool Planner::get_index_range_cols(const std::string& tab_name, std::vector<Condition>& curr_conds, std::vector<std::string>& index_col_names) {
    index_col_names.clear();
    std::map<int,int> permutation;
    std::vector<int> vis_mask(curr_conds.size(), 0);
    TabMeta& tab = sm_manager_->db_.get_table(tab_name);

    bool ret = tab.is_leftmost_range_match(curr_conds, vis_mask, permutation, index_col_names);
    ...
    int begin_ptr = permutation.size();
    for(auto iter = curr_conds.begin(); iter!= curr_conds.end(); iter++){
        //vis_mask: 不在索引中的字段为0, 若有多个取第一个, 只允许忽略索引的第一个字段且索引第一个字段必须为int
        if(vis_mask[iter- curr_conds.begin()] == 0){
            permutation[begin_ptr++] = iter - curr_conds.begin();
        }
    }
...
std::shared_ptr<Query> Planner::logical_optimization(std::shared_ptr<Query> query, Context *context)
{
    ...
    //TODO 实现逻辑优化规则
    std::unordered_map<std::string, Value*> conds_vals;
    //添加到conds_vals
    for(auto iter = query->conds.begin(); iter!= query->conds.end(); ++iter){
        if(iter->op == CompOp::OP_EQ && iter->is_rhs_val){
            conds_vals[iter->lhs_col.tab_name + "$" + iter->lhs_col.col_name] = (&(iter->rhs_val));
        }
    }
    bool is_add = false;
    Condition cond;
    int len = -1;
    for(auto iter = query->conds.begin(); iter!= query->conds.end(); ++iter){
        if(!iter->is_rhs_val && iter->op == CompOp::OP_EQ){
            auto l_it = conds_vals.find(iter->lhs_col.tab_name + "$" + iter->lhs_col.col_name);
            auto r_it = conds_vals.find(iter->rhs_col.tab_name + "$" + iter->rhs_col.col_name);
            bool l_exists = (l_it != conds_vals.end());
            bool r_exists = (r_it != conds_vals.end());
            if(l_exists && !r_exists){
                cond.lhs_col = {.tab_name = iter->rhs_col.tab_name, .col_name = iter->rhs_col.col_name};
                cond.op = CompOp::OP_EQ;
                cond.is_rhs_val = true;
                cond.rhs_val.copy_from(*(l_it->second));
                is_add = true;
                len = l_it->second->raw->size;
                break;
            }
            else if(!l_exists && r_exists){
                cond.lhs_col = {.tab_name = iter->lhs_col.tab_name, .col_name = iter->lhs_col.col_name};
                cond.op = CompOp::OP_EQ;
                cond.is_rhs_val = true;
                cond.rhs_val.copy_from(*(r_it->second));
                is_add = true;
                len = r_it->second->raw->size;
                break;
            }
        }
    }
    if(is_add){
        query->conds.emplace_back(cond);
        query->conds.back().rhs_val.init_raw(len);
    }
...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query> query)
{
    ...
    for (size_t i = 0; i < tables.size(); i++) {
        ...
        // } 
        if (index_exist == false) {  // 该表没有索引
            bool index_range_exist = get_index_range_cols(tables[i], curr_conds, index_col_names);
            if(index_range_exist){
                table_scan_executors[i] = std::make_shared<ScanPlan>(T_IndexRangeScan, sm_manager_, tables[i], curr_conds, index_col_names);
            }
            else{
                index_col_names.clear();
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    for(auto iter = x->order->cols.begin(); iter!= x->order->cols.end(); iter++){
        is_descs.emplace_back(iter->get()->order_by_dir == ast::OrderBy_DESC);
    }
    return std::make_shared<SortPlan>(T_Sort, std::move(plan), query->order_cols, 
                                    is_descs, x->order->limit_cnt);
}
...
std::shared_ptr<Plan> Planner::generate_select_plan(std::shared_ptr<Query> query, Context *context) {
    //逻辑优化 
    query = logical_optimization(std::move(query), context);//TODO

    //物理优化
    auto sel_cols = query->cols;
    std::shared_ptr<Plan> plannerRoot = physical_optimization(query, context);//TODO
    plannerRoot = std::make_shared<ProjectionPlan>(T_Projection_Nocopy, std::move(plannerRoot), 
                                                        std::move(sel_cols));
    
    //如果使用了聚集函数
    if(query->agre_types.size() != 0) {
        plannerRoot = std::make_shared<AggregationPlan>(T_Aggregation, std::move(plannerRoot), query->agre_types, query->agre_cols, query->target_cols);
        return plannerRoot;
    }
    return plannerRoot;
}
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query, Context *context)
{
    ...
    if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
        ...
        if (index_exist == false) {  // 该表没有索引
            bool index_range_exist = get_index_range_cols(x->tab_name, query->conds, index_col_names);
            if(index_range_exist){
                table_scan_executors = std::make_shared<ScanPlan>(T_IndexRangeScan, sm_manager_, x->tab_name, query->conds, index_col_names);
            }
            else{
                index_col_names.clear();
        ...
        // 生成select语句的查询执行计划
        std::shared_ptr<Plan> planroot = generate_select_plan(std::move(query), context);
        plannerRoot = std::make_shared<DMLPlan>(T_select, planroot, std::string(), std::vector<Value>(),
