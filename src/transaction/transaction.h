...
private:
    ...
    std::shared_ptr<std::deque<std::pair<Page*, int>>> index_latch_page_set_;      // 维护事务执行过程中加锁的索引结点
...
public:
    explicit Transaction(txn_id_t txn_id, IsolationLevel isolation_level = IsolationLevel::SERIALIZABLE)
        ...
        index_latch_page_set_(std::make_shared<std::deque<std::pair<Page*, int>>>())
    {
    ...
    inline std::shared_ptr<std::deque<std::pair<Page*, int>>>  get_index_latch_page_set() { return index_latch_page_set_; }
    inline void append_index_latch_page_set(Page* page, int op) { index_latch_page_set_->emplace_back(std::make_pair(page, op)); }

    inline std::shared_ptr<std::unordered_set<LockDataId>> get_lock_set() { return lock_set_; }
    inline void append_lock_set(LockDataId lock_id) { lock_set_->insert(lock_id); }
    inline void erase_lock_set(LockDataId lock_id) { lock_set_->erase(lock_id); }
