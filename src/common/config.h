...
#ifdef DEBUG_CONFIG
    static constexpr int BUFFER_POOL_SIZE = 40;
    // Use for debug
#endif
#ifndef DEBUG_CONFIG
    // static constexpr int BUFFER_POOL_SIZE = 65536 * 2;                                // size of buffer pool 256MB(DEFAULT)
    static constexpr int BUFFER_POOL_SIZE = 262144 * 4;                                // size of buffer pool 1GB
    // static constexpr int BUFFER_POOL_SIZE = (262144 * 3) /2;                             // size of buffer pool 2GB
    // static constexpr int LOG_BUFFER_SIZE = (1024 * PAGE_SIZE);                    // size of a log buffer in byte
#endif

static constexpr int INDEX_BUFFER_POOL_SIZE = BUFFER_POOL_SIZE / 8;

#ifdef DEBUG_LOG_MANAGER
    static constexpr int LOG_BUFFER_SIZE = PAGE_SIZE;
#endif

#ifndef DEBUG_LOG_MANAGER
    // static constexpr int LOG_BUFFER_SIZE = (1024 * PAGE_SIZE);
    static constexpr int LOG_BUFFER_SIZE = 16 * (1024 * PAGE_SIZE); // 4MB
#endif
