...
struct RmPageHandle {
    ...
    RmPageHandle(const RmFileHdr *fhdr_, Page *page_) : file_hdr(fhdr_), page(page_) {
        ...
        bitmap = page->get_data() + sizeof(RmPageHdr) + page->OFFSET_PAGE_HDR;//Problem OFFSET_PAGE_HDR 4
...
class RmFileHandle {      
   ...
   private:
    ...
    std::mutex latch_; //临时latch, 在树协议之后需要替换.
    std::set<page_id_t> free_pageno_set;
    //性能优化用，存储一些常用的变量避免频繁申请空间
    PageId tmp_new_page_id;
   public:
    BufferPoolManager *buffer_pool_manager_;
    RmFileHandle(DiskManager *disk_manager, BufferPoolManager *buffer_pool_manager, int fd)
        : disk_manager_(disk_manager), fd_(fd), buffer_pool_manager_(buffer_pool_manager){
        ...
        tmp_new_page_id.fd = fd_;
    }

    int get_record_size() {
        return file_hdr_.record_size;
    }
    ...
    bool is_record(const Rid &rid) const {
        unpin_page_guard upg({fd_, rid.page_no}, false, buffer_pool_manager_);
    ...
    std::unique_ptr<RmRecord> get_record(const Rid &rid, bool is_add_lock, Context *context) const;

    Rid insert_record(char *buf, Context *context);

    void insert_record(Rid &rid, char *buf);

    Rid insert_record_for_load_data(char* buf, RmPageHandle& page_buffer);

    void delete_record(Rid &rid, Context *context);

    void delete_record(Rid &rid, bool is_add_lock, Context *context);

    void update_record(Rid &rid, char *buf, Context *context);

    void update_record(Rid &rid, char *buf, bool is_add_lock, Context *context);
