...
#include "defs.h"
#define SHOW_IO
// #define ENABLE_ASSERT_DISKMANAGER
// #define DEBUG_DISK_MANAGER
...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    int return_lseek = lseek(fd, page_no * PAGE_SIZE, SEEK_SET);
    if(return_lseek == -1) {
        assert(0&&"ERROR: DiskManager::write_page lseek error");
    }
    int return_num = write(fd, offset, num_bytes);
    // int ret_sync = fsync(fd);
    // if(ret_sync==-1){
    //     #ifdef ENABLE_ASSERT_DISKMANAGER
    //         assert(0);
    //     #endif
    // }
    if(return_num != num_bytes) {
        throw InternalError("ERROR: DiskManager::write_page Error");
    }
    #ifdef SHOW_IO
        this->IO_cnt ++;
    #endif
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    int return_lseek = lseek(fd, page_no * PAGE_SIZE, SEEK_SET);
    if(return_lseek == -1) {
        assert(0&&"ERROR: DiskManager::read_page lseek error");
    }
    int return_num = read(fd, offset, num_bytes);
    if(return_num != num_bytes) {
...
void DiskManager::create_file(const std::string &path) {
    ...
    int fd = open(path.c_str(), O_CREAT, S_IRWXU);
    if(fd==-1) {
        assert(0&&"ERROR: DiskManager::create_file file open() error");
    }
    close(fd);
}
...
int DiskManager::open_file(const std::string &path) {
    ...
    if(path2fd_.find(path) != path2fd_.end()) {
        // throw FileNotFoundError(path);
        assert(0 && "ERROR: DiskManager::open_file already open file");
    }
...
void DiskManager::close_file(int fd) {
    ...
    if(fd2path_.find(fd) == fd2path_.end()) {
        assert(0&&"ERROR: DiskManager::close_file file not opened");
    }
    int is_close = close(fd);
    if(is_close==-1) {
        assert(0&&"ERROR: DiskManager::close_file close() error");
    }
...
int DiskManager::get_file_size(const std::string &file_name) {
    ...
    #ifdef DEBUG_DISK_MANAGER
        char error_message[1024];
        strncpy(error_message, strerror(errno), sizeof(error_message) - 1);
        error_message[sizeof(error_message) - 1] = '\0';
        std::string error_(error_message);
    #endif
...
void DiskManager::write_log(char *log_data, int size) {
    ...
    lseek64(log_fd_, 0, SEEK_END);
    ssize_t bytes_write = write(log_fd_, log_data, size);
    // int sync_result = fsync(log_fd_);
    // if(sync_result == -1) {
    //     #ifdef ENABLE_ASSERT_DISKMANAGER
    //         assert(0);
    //     #endif
