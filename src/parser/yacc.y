...
%type <sv_node> stmt dbStmt ddl dml txnStmt io_stmt
%type <sv_field> field
...
%type <sv_type_agre> agreType
%type <sv_agrecol> agreCol
%type <sv_agrecols> agreColList
%type <sv_order_col> order_col
%type <sv_order_cols> order_cols
...
dml:
    ...
    |   SELECT agreColList FROM tableList optWhereClause opt_order_clause
    {
        $$ = std::make_shared<SelectStmt>($2, $4, $5, $6);
    }
    ;
...
type:
    ...
    |   DATETIME
    {
        $$ = std::make_shared<TypeLen>(SV_TYPE_DATETIME, 19); /*make it enough large.*/
    }
    ;
...
agreCol:
        agreType '(' col ')' AS colName
    {
        $$ = std::make_shared<AgreCol>($1, $3, $6);
    }
    |   agreType '(' '*' ')' AS colName
    {
        $$ = std::make_shared<AgreCol>($1, std::make_shared<Col>("",""), $6);
    }
    ;
...
agreColList:
        agreCol
    {
        $$ = std::vector<std::shared_ptr<AgreCol>>{$1};
    }
    |   agreColList ',' agreCol
    {
        $$.push_back($3);
...
setClause:
        colName '=' value
    {
        $$ = std::make_shared<SetClause>($1, $3, SetOperation::ASSIGN);
    }
    |   colName '=' colName value 
    {
        $$ = std::make_shared<SetClause>($1, $4, SetOperation::PLUS);
    }
    |   colName '=' colName '+' value 
    {
        $$ = std::make_shared<SetClause>($1, $5, SetOperation::PLUS);
    }
    |   colName '=' colName '-' value 
    {
        $$ = std::make_shared<SetClause>($1, $5, SetOperation::MINUS);
    }
    ;
