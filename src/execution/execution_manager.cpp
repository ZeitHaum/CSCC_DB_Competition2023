...
#include "executor_index_range_scan.h"
...
#define ENABLE_PRINT //define for enable printer
...
void QlManager::run_cmd_utility(std::shared_ptr<Plan> plan, txn_id_t *txn_id, Context *context) {
    if (auto x = std::dynamic_pointer_cast<OtherPlan>(plan)) {
        switch(x->tag) {
            ...
            case T_LoadData:
            {
                #ifndef ENABLE_LOAD_ITER
                    sm_manager_ -> load_data(x->tab_name_, x->file_name_);
                #endif
                #ifdef ENABLE_LOAD_ITER
                    sm_manager_ -> load_csv_itermodel(x->file_name_, x->tab_name_);
                #endif
                break;
            }
            case T_IoEnable:
