...
// auto index_buffer_pool_manager = std::make_unique<BufferPoolManager>(INDEX_BUFFER_POOL_SIZE, disk_manager.get(), log_manager.get());
...
auto ql_manager = std::make_unique<QlManager>(sm_manager.get(), disk_manager.get(), txn_manager.get());
...
void sigint_handler(int signo) {
    should_exit = true;
    log_manager->flush_log_to_disk(FlushReason::CLOSE_DB);
...
void *client_handler(void *sock_fd) {
    ...
    while (true) {
        ...
        if (i_recvBytes == 0) {
            std::cout << "Maybe the client has closed" << '\n';
            break;
        }
        if (i_recvBytes == -1) {
        ...
        Context *context = new Context(txn_manager.get(), lock_manager.get(), log_manager.get(), nullptr, data_send, &offset);
            ...
            if (yyparse() == 0) {
                if (ast::parse_tree != nullptr) {
                    try {
                        // analyze and rewrite
                        // assert(0);
                        std::shared_ptr<Query> query = analyze->do_analyze(ast::parse_tree); // wsh: 检查语义是否正确
                        ...
                        portal->drop();
                        #ifdef DEBUG
                            std::cout<<"DiskManager IoCount is "<<disk_manager->IO_cnt<<"\n";
                        #endif
                        ...
                        if(sm_manager ->io_enabled_){
                                ...
                                disk_manager->outfile << str;
                                disk_manager->outfile.flush();
                            #endif
                        }
                    } catch (RMDBError &e) {
    ...
    // Clear
    std::cout << "Terminating current client_connection..." << '\n';
    delete[] data_send;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        recovery->undo();
        #ifndef ENABLE_OUTPUTFILE_IO
            disk_manager->outfile.open("output.txt", std::ios::out | std::ios::app);
        #endif
