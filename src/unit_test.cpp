...
std::ostream &operator<<(std::ostream &os, std::vector<PageId>& vec){
    os<< "[";
    for(auto i : vec){
        os<<i.page_no<<",";
    }
...
TEST_F(BufferPoolManagerConcurrencyTest, ConcurrencyTest) {
    ...
    for (int run = 0; run < num_runs; run++) {
        ...
        for (int tid = 0; tid < num_threads; tid++) {
            threads.emplace_back(std::thread([&bpm, fd]() {  // NOLINT
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    page_ids.emplace_back(temp_page_id);
                } 
                for (int i = 0; i < 10; i++) {
